...
function renderPartContent(paragraphs, pullQuotes) {
  const elements = [];
  let quoteIdx = 0;
  ...
  for (let i = 0; i < paragraphs.length; i += CHUNK_SIZE) {
    ...
    const hasMoreParagraphs = i + CHUNK_SIZE < paragraphs.length;
...
export default function LenaOkaforPage() {
  const story = lenaOkaforStory;
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          >
            <Link href="/stories" className="text-sm hover:underline" style={{ color: '#2E6DB4' }}>
              ← All Stories
            </Link>
          </motion.div>
        ...
        <div className="max-w-3xl mx-auto px-6 py-16">
          {story.parts.map((part, partIndex) => (
            <div key={partIndex}>
              ...
              {renderPartContent(part.paragraphs, part.pullQuotes)}
            </div>
          ))}
        </div>
      </div>
